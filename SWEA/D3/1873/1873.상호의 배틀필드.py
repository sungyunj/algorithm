...
if T_str:
    ...
    for t in range(1, T + 1):
        ...
        # 1. 전차의 초기 위치(r, c)와 현재 방향(d) 찾기
        r, c, d = 0, 0, ''
        found = False
        for i in range(H):
            for j in range(W):
                if board[i][j] in shape_dir:
                    r, c = i, j
                    d = shape_dir[board[i][j]]
                    found = True
                    break
            if found: break
        ...
        for cmd in commands:
            if cmd == 'S':
                ...
                while 0 <= ny < H and 0 <= nx < W:
                    if board[ny][nx] == '*': # 벽돌 벽: 파괴 후 평지화
                        board[ny][nx] = '.'
                        break
                    elif board[ny][nx] == '#': # 강철 벽: 멈춤
                        break
                    ny += dy
                    nx += dx
            else:
                ...
                d = cmd
                dy, dx, shape = move[d]
                board[r][c] = shape # 먼저 현재 위치에서 방향 전환
                
                ny, nx = r + dy, c + dx
                # 이동 가능 여부 체크 (범위 내 + 평지)
                if 0 <= ny < H and 0 <= nx < W and board[ny][nx] == '.':
                    board[r][c] = '.' # 기존 위치는 평지로
                    r, c = ny, nx
                    board[r][c] = shape # 새 위치로 이동

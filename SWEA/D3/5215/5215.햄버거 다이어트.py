...
# 다음 N개의 줄에는 재료에 대한 민기의 맛에 대한 점수와 칼로리를 나타내는 T_i, K_i(1 ≤ T_i ≤ 10^3, 1 ≤ K_i ≤ 10^3)가 공백으로 구분되어 주어진다.
...
for tc in range(1, T + 1):
    ...
    # dp[j] 는 j 칼로리로 만들 수 있는 최대 점수
    dp = [0] * (L + 1)
    
    for _ in range(N):
        score, kcal = map(int, input().split())
        # 뒤에서부터 계산해야 같은 재료를 중복 사용하는 것을 방지할 수 있음

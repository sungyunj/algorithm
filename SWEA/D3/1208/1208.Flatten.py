...
import sys
sys.stdin = open("/Users/tjddbsj/Desktop/github/algorithm/algorithm/SWEA/D3/1208/input.txt", "r")

for test_case in range(1, 11):
    # 1. 입력 받기
    dump = int(input().strip())  # 덤프 가능 횟수
    heights = list(map(int, input().split()))  # 상자들의 높이 리스트 (길이 100)
    # 2. 덤프 작업 반복
    for _ in range(dump):
        # 현재 상태에서 최고점과 최저점의 인덱스 찾기
        max_idx = heights.index(max(heights))
        min_idx = heights.index(min(heights))

        # 최고점과 최저점의 차이가 1 이하라면 이미 평탄화 완료
        if heights[max_idx] - heights[min_idx] <= 1:
            break
    ...
    # 3. 최종 결과 계산 (반복이 끝난 후 다시 한 번 최고/최저 차이를 구함)

# [제한 사항]
# 시간: 10개 테스트케이스를 합쳐서 C++ 의 경우 10초 / Java 의 경우 20초 / Python 의 경우 30초
...
def bfs():
    # 100x100 크기이므로 N을 입력받아 사용
    # distance 배열을 매우 큰 값(INF)으로 초기화.
    dist = [[float('inf')] * N for _ in range(N)]
    
    queue = deque([(0, 0)]) # 시작점 (0, 0)
    dist[0][0] = maze[0][0] # 시작점 비용 (문제에서는 0)
    ...
    while queue:
        ...
        for i in range(4):
            ...
            if 0 <= ny < N and 0 <= nx < N:
                # (현재까지의 누적 합 + 다음 칸의 복구 시간)이 
                # 이전에 기록된 다음 칸의 최소 비용보다 작다 -> 갱신
                if dist[y][x] + maze[ny][nx] < dist[ny][nx]:

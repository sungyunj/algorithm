...
import sys
sys.stdin = open("/Users/tjddbsj/Desktop/github/algorithm/algorithm/SWEA/D4/1238/input.txt","r")
...
for tc in range(1, 11):
    ...
    # {from, to, from, to...} 형태의 데이터 읽기
    # data = list(map(int, sys.stdin.readline().split()))
    data = list(map(int, input().split()))
    
    # 1. 인접 리스트로 그래프 만들기
    adj = [[] for _ in range(101)]
    for i in range(0, N, 2): #{from, to} 쌍이기 때문에 2칸씩 건너뛰면서
        u, v = data[i], data[i+1]
        adj[u].append(v)
        
    # 2. BFS 탐색 준비
    # visited[i]는 i번 사람이 연락을 받은 '단계(시간)'를 저장함
    visited = [0] * 101
    queue = deque([S])
    visited[S] = 1 # 시작점(당번)은 1단계로 표시
    
    while queue:
        curr = queue.popleft()
        
        # 현재 사람(curr)과 연결된 다음 사람(next_node) 확인
    ...
    # 3. 결과 찾기
    max_val = 0 # 가장 늦은 시간(최대 단계)
    ans = 0     # 그 중 가장 큰 번호
    
    # visited 배열을 돌며 가장 큰 단계와 그 때의 인덱스(사람 번호)를 찾음
